 ...
 *
 * <p>Supports identity preservation during cross-persistence sync.
 * Manages user notifications with read status tracking.</p>
 *
 * @author Elia Cinti
 * @version 1.0
 */
public class NotificationDaoMySql extends AbstractMySqlDao implements NotificationDao {
    ...
    public void markAllAsReadForUser(String u, UserType t) throws DAOException {
        try {
            Connection conn = ConnectionFactory.getConnection();
            try (PreparedStatement stmt = conn.prepareStatement(SQL_MARK_ALL_READ)) {
                stmt.setString(1, u);
